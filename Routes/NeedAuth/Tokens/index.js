...
router.post('/getchanneltoken', async (req, res) => { //TODO: Make this a GET
...
router.get('/getrtmtoken', (req, res) => {
    const phone_number = req.user['https://backend.letsusespeakup.com/token/usermetadata/phone_number']
        || req.user['https://backend.letsusespeakup.com/token/usermetadata/metadata'].phone_number;
    ...
    const currentTimestamp = Math.floor(Date.now() / 1000)
    const timeToLive = 30 * 60 * 24;
    const privilegeExpiredTs = currentTimestamp + timeToLive;

    const token = RtmTokenBuilder.buildToken(APP_ID, APP_CERTIFICATE, phone_number, RtmRole.Rtm_User, privilegeExpiredTs);
    return res.status(200).send({
        token: token
    });
})
